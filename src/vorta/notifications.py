...
class DBusNotifications(VortaNotifications):
    ...
    def _dbus_notify(self, header, msg, level='info'):
        """
        Send a notifcation over the freedesktop dbus interface.

        The interface is documented here:
        https://www.galago-project.org/specs/notification/0.9/x408.html

        Parameters
        ----------
        header : str
            The title/summary line of the notification.
        msg : str
            The body of the notification.
        level : str, optional
            The urgency of the notification, by default 'info'
        """
        # dbus identifiers
        ...
        app_name = "Vorta"
        notification_id = QVariant(0)  # unset
        notification_id.convert(QMetaType.Type.UInt)
        icon = "com.borgbase.Vorta"
        title = header
        text = msg
        actions_list = QVariant([])
        actions_list.convert(QMetaType.QStringList)
        hint = {'urgency': self.URGENCY[level], 'desktop-entry': "com.borgbase.Vorta"}
        time = -1  # unset
